        ...
        public partial class IL2CPP
        {
            public partial class Instance
            {
                internal class InstanceDestroy
                {
                    ...
                    private static readonly int SubToolGeneralLimit = 15000;

                    private static readonly ulong AllocateSize = 0x10000;
                    private static ulong AllocateStart = 0;

                    private static ulong AddressFreeUse = 0;
                    private static ulong AddressArguments = 0;
                    private static ulong AddressArgumentsData = 0;
                    ...
                    internal void AddArgument(ulong instancePtr, short instances)
                    {
                        do
                        {
                            ...
                            {
                                Main.ProcessInstance.WriteBytes((nint)AddressArguments, BitConverter.GetBytes(instancePtr + (ulong)(i * 0x8)));
                    ...
                    private Result ScanData()
                    {
                        ...
                        try
                        {
                            do
                            {
                                ...
                                ulong il2cpp_gc_wbarrier_set_field = TProcess.GetProcAddress(Main.ProcessInstance, "GameAssembly.dll", "il2cpp_gc_wbarrier_set_field");
                                if (il2cpp_gc_wbarrier_set_field == 0)
                                    break;

                                ulong moduleBase = TProcess.GetModuleBase(Main.ProcessInstance, "UnityPlayer.dll");
                                if (moduleBase == 0)
                                    break;

                                List<ulong> results = [.. TMemory.ScanMultiple(Main.ProcessInstance, "48 83 C4 ?? 5B C3 48", "UnityPlayer.dll", 0x20)];
                                    ...
                                    for (int i = 0; i < results.Count; i++)
                                    {
                                        try
                                        {
                                            ...
                                            if (functionEnd != 0)
                                            {
                                                ...
                                                {
                                                    // is searched call
                                                    if (!op.ToString().StartsWith("call "))
                                                        continue;
                                                    if (op.Bytes[0] != 0xFF || op.Bytes[1] != 0x15)
                                                        continue;
                                                    ...
                                                    try
                                                    {
                                                        relative = TMemory.ReadMemory<ulong>(Main.ProcessInstance, relativePtr);
                                                    }
                                                    catch { }

                                                    if (relative != il2cpp_gc_wbarrier_set_field)
                                                        continue;
                                ...
                                if (results.Count == 1)
                                {
                                    try
                                    {
                                        ...
                                        {
                                            CallFinalSetField = setFieldCalls[^3];
                                        }
                                    }
                                    catch { }
                                }
                    ...
                    private Result WriteArgs()
                    {
                        ...
                        do
                        {
                            ...
                            Main.ProcessInstance.WriteBytes((nint)(AllocateStart + GeneratedOffsets.il2cpp_gc_wbarrier_set_field), BitConverter.GetBytes(TProcess.GetProcAddress(Main.ProcessInstance, "GameAssembly.dll", "il2cpp_gc_wbarrier_set_field")));
                            result = Result.Success;
                        }
                        while (false);
                        TUtils.Print(DebugClass + "." + GetType().Name + "." + MethodBase.GetCurrentMethod().Name +

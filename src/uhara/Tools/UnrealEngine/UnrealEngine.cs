        ...
        public string FNameToString(uint fName)
        {
            try
            {
                do
                {
                    if (!Obsolete1Shown)
                    {
                        TUtils.Print("UnrealEngine." + GetType().Name + "." + MethodBase.GetCurrentMethod().Name +
                        " | " + " Iherited FNameToString() is obsolete, use FNameToString() in tool (\"UnrealEngine\", \"Utils\")");
                        Obsolete1Shown = true;
                    }

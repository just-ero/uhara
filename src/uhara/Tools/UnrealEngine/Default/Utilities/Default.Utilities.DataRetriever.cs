                ...
                public class DataRetriever
                {
                    public nint FindData(string dataName)
                    {
                        try
                        {
                            ...
                            else if (dataNameLower is "gengine" or "engine" or "gameengine")
                            {
                                address = TConvert.Parse<ulong>(ProcessCache.Get(dataNameLower));

                                if (address == 0)
                                    address = TMemory.ScanRel(Main.ProcessInstance, 8, "E8 ???????? 48 8B 0D ???????? 49 8B ?? 48 8B 01 FF 90 ???????? 48 8?");
                                if (address == 0)
                                    address = TMemory.ScanRel(Main.ProcessInstance, 3, "48 89 05 ???????? 48 85 C9 74 ?? E8 ???????? 48 8D 4D");
                                if (address == 0)
                            ...
                            else if (dataNameLower is "gworld" or "world")
                            {
                                ...
                                if (address == 0)
                                    address = TMemory.ScanRel(Main.ProcessInstance, 3, "48 8B 1D ?? ?? ?? ?? 48 85 DB 74 ?? 41 B0 01");
                                if (address == 0)
                            ...
                            else if (dataNameLower is "gsync" or "gsyncload" or "gsyncloadcount")
                            {
                                ...
                                if (address == 0)
                                    address = TMemory.ScanRel(Main.ProcessInstance, 5, "89 43 60 8B 05 ?? ?? ?? ?? 89");
                                if (address == 0)
                                {
                                    do
                                    {
